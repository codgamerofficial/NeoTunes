...
export default function MiniPlayer() {
  ...
  return (
    <>
      {/* PERSISTENT APPLICATION CONTROL BAR (Specs 1, 2, 4, 10) */}
      <footer
        className={`fixed bottom-16 md:bottom-0 left-0 ${leftPositionClass} right-0 h-[72px] md:h-[88px] z-40 bg-[#070A14]/95 backdrop-blur-2xl border-t border-white/10 px-4 md:px-6 select-none shadow-[0_-10px_30px_rgba(0,0,0,0.8)] transition-all duration-300 flex items-center justify-between font-sans`}
      >
        ...
        <div className="flex items-center gap-3.5 min-w-0 w-1/4 max-w-[280px]">
          ...
          <div className="min-w-0 flex-1">
            <h4 
              onClick={() => router.push('/player')}
              className="font-black text-xs md:text-sm text-white truncate hover:text-[#00D4FF] transition-colors cursor-pointer"
            >
              {currentTrack.title}
            </h4>
        ...
        {/* ── CENTER COLUMN: TRANSPORT CONTROLS & SEEK SCRUBBER ── */}
        <div className="flex flex-col items-center justify-center min-w-0 flex-1 max-w-[580px] px-2 md:px-6 space-y-1">
          {/* Transport Buttons */}
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            {/* Compact Hero Play/Pause Button (44-48px) */}
            <button
